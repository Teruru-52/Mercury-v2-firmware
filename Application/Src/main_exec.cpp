...
void UpdateUndercarriage() { controller.UpdateOdometory(); }
...
void StateProcess() {
  if (state.mode_ == State::select_function) {
    ...
    if (ir_sensors.StartInitialize()) Initialize();
  }
    ...
    switch (state.mode_) {
      ...
      case State::test_rotation:  // func13
        for (int i = 0; i < 10; i++) controller.PivotTurn(90);
        // for (int i = 0; i < 4; i++)
        //     controller.PivotTurn(180);
      ...
      case State::test_odometory:  // func14
        static int cnt = 0;
        // controller.MotorTest(0.0f, 3.0f * sin(cnt * 0.0000628));
        controller.MotorTest(3.0f * sin(cnt * 0.0000628), 0.0f);
        cnt = (cnt + 1) % 100000;
        break;
...
bool GetStartCountFlag() { return flag_start_cnt; }
