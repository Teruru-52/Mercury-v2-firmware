...
void DynamicFeedback::UpdateRef(const ctrl::Pose& ref_p,
                                const ctrl::Pose& ref_v,
                                const ctrl::Pose& ref_a) {
  ref_pos = ref_p;
...
ctrl::Pose DynamicFeedback::CalcInput(const ctrl::Pose& cur_pos,
                                      const ctrl::Pose& cur_v) {
  const float cos_th = arm_cos_f32(cur_pos.th);
  const float sin_th = arm_sin_f32(cur_pos.th);
  ...
  const float ux =
      ref_acc.x + kd * (ref_vel.x - cur_vel.x) + kp * (ref_pos.x - cur_pos.x);
  const float uy =
      ref_acc.y + kd * (ref_vel.y - cur_vel.y) + kp * (ref_pos.y - cur_pos.y);
  ...
  if (abs(xi) < xi_threshold) {
    ...
    const auto k2 = b;
    const auto k3 = k1;
    ref_u.x = v_d * arm_cos_f32(cos_th_r - cos_th) +
              k1 * (cos_th * (ref_pos.x - cur_pos.x) +
                    sin_th * (ref_pos.y - cur_pos.y));
    ref_u.th = w_d +
               k2 * v_d * sinc(ref_pos.th - cur_pos.th) *
                   (cos_th * (ref_pos.x - cur_pos.x) -
                    sin_th * (ref_pos.y - cur_pos.y)) +
               k3 * (ref_pos.th - cur_pos.th);
  } else {
    ref_u.x = xi;
...
void TimeVaryingFeedback::UpdateRef(const ctrl::Pose& ref_p,
                                    const ctrl::Pose& ref_v,
                                    const ctrl::Pose& ref_a) {
  ref_pos = ref_p;
  cos_th_r = arm_cos_f32(ref_pos.th);
...
ctrl::Pose TimeVaryingFeedback::CalcInput(const ctrl::Pose& cur_pos,
                                          const ctrl::Pose& cur_v) {
  const float cos_th = arm_cos_f32(cur_pos.th);
  const float sin_th = arm_sin_f32(cur_pos.th);
