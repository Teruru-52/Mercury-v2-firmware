...
const float ir_start_base = 3200;
...
// for wall judgement
hardware::IR_Base ir_is_wall = {ir_fl1_wall, ir_fl2_wall, ir_fl3_wall,
                                ir_fr1_wall, ir_fr2_wall, ir_fr3_wall};
// for front/side wall correction
hardware::IR_Base ir_ctrl_base = {ir_fl1_base, ir_fl2_base, ir_fl3_base,
                                  ir_fr1_base, ir_fr2_base, ir_fr3_base,
                                  ir_slalom};
// hardware::IR_LogCoeff ir_log = {.a = 41.01f, .b = 1.314e-5f, .c = -0.02817f,
// .d = 262.2f};
hardware::IR_LogCoeff ir_log = {
    .a = 41.0f, .b = 1.3e-05f, .c = -0.028f, .d = 262.1f};
...
// PID pid_rotational_vel(1.1976f, 85.1838f, -0.00099f, 0.0039227f,
// control_period); PID pid_rotational_vel(1.3f, 30.0f, 0.001f, 0.0004f,
// control_period);
PID pid_rotational_vel(0.3f, 30.0f, 0.0f, 0.0004f, control_period);
// PID pid_traslational_vel(0.0068176f, 0.0820249f, -0.000033349f, 0.023191f,
// control_period);
...
PID_Instances pid(&pid_angle, &pid_rotational_vel, &pid_traslational_vel,
                  &pid_ir_front_left, &pid_ir_front_right, &pid_ir_side);
...
trajectory::Parameter slalom_param1 = {
    .v_max = 2.0f * M_PI, .a_max = 30.0f * M_PI, .j_max = 1.0e+3f * M_PI};
trajectory::Parameter slalom_param2 = {
    .v_max = 5.0f * M_PI, .a_max = 40.0f * M_PI, .j_max = 1.5e+3f * M_PI};
trajectory::Parameter slalom_param3 = {
    .v_max = 8.0f * M_PI, .a_max = 50.0f * M_PI, .j_max = 1.5e+3f * M_PI};
trajectory::Parameter slalom_param4 = {
    .v_max = 12.0f * M_PI, .a_max = 60.0f * M_PI, .j_max = 2.0e+3f * M_PI};
trajectory::Parameter slalom_param5 = {
    .v_max = 15.0f * M_PI, .a_max = 70.0f * M_PI, .j_max = 2.5e+3f * M_PI};
trajectory::Parameters slalom_params = {.run1 = slalom_param1,
                                        .run2 = slalom_param2,
                                        .run3 = slalom_param3,
                                        .run4 = slalom_param4,
                                        .run5 = slalom_param5};
...
undercarriage::Controller controller(&odom, &pid, tracker, &slalom, &acc,
                                     &ir_param, &velocity);