...
IRsensor::IRsensor(float ir_start_base, IR_Base* ir_is_wall)
    : ir_start_base(ir_start_base), ir_is_wall(ir_is_wall) {}
...
void IRsensor::StartDMA() { HAL_ADC_Start_DMA(&hadc1, (uint32_t*)dma_ir, 6); }

void IRsensor::UpdateLeftValue() {
  if (dma_ir[2] > max_fl1) max_fl1 = dma_ir[2];
  if (dma_ir[1] > max_fl2) max_fl2 = dma_ir[1];
  if (dma_ir[0] > max_fl3) max_fl3 = dma_ir[0];
}

void IRsensor::UpdateRightValue() {
  if (dma_ir[5] > max_fr1) max_fr1 = dma_ir[5];
  if (dma_ir[4] > max_fr2) max_fr2 = dma_ir[4];
  if (dma_ir[3] > max_fr3) max_fr3 = dma_ir[3];
}
...
void IRsensor::PrintLog() {
  printf("%lu, %lu, %lu, %lu, %lu, %lu\n", ir_value.fl1, ir_value.fl2,
         ir_value.fl3, ir_value.fr1, ir_value.fr2, ir_value.fr3);
}
}  // namespace hardware