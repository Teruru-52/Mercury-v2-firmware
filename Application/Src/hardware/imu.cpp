...
#define SPI_ACCEL_YOUT_H 0x3B
#define SPI_ACCEL_YOUT_L 0x3C
...
void IMU::CalcOffset() {
  ...
  for (int i = 0; i < 500; i++) {
    // H:8bit shift, Link h and l
    ax_raw = (int16_t)((int16_t)(read_byte(SPI_ACCEL_YOUT_H) << 8) |
                       read_byte(SPI_ACCEL_YOUT_L));
...
float IMU::GetAngularVelocity() { return gyro_z; }

float IMU::GetAccX() { return acc_x * g; }
