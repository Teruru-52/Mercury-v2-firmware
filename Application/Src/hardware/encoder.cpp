...
namespace hardware {
Encoder::Encoder(float sampling_period) : sampling_period(sampling_period) {
...
int16_t Encoder::GetPulseL() {
  ...
  int16_t enc_buff = TIM3->CNT;
  pulse_left = (int16_t)enc_buff;
  if (pulse_left < 0) pulse_left += 32767;
  __enable_irq();
  return pulse_left;
}
...
int16_t Encoder::GetPulseR() {
  ...
  int16_t enc_buff = TIM1->CNT;
  pulse_right = (int16_t)enc_buff * -1;
  if (pulse_right < 0) pulse_right += 32767;
  __enable_irq();
  return pulse_right;
}
...
// {
//     return (GetAngularVelocity(pulse_left) + GetAngularVelocity(pulse_right))
//     * tire_radius * 0.5f;
// }

float Encoder::GetVelocity() {
  return (GetAngularVelocity(pulse_left) * tire_radius_l +
          GetAngularVelocity(pulse_right) * tire_radius_r) *
         0.5f;
}
