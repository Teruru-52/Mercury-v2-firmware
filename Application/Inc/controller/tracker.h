...
class TrackerBase {
 ...
 public:
  ...
  virtual void UpdateRef(const ctrl::Pose& ref_p, const ctrl::Pose& ref_v,
                         const ctrl::Pose& ref_a) = 0;
  virtual ctrl::Pose CalcInput(const ctrl::Pose& cur_pos,
                               const ctrl::Pose& cur_v) = 0;
...
class Kanayama : public TrackerBase {
 ...
 public:
  ...
  void UpdateRef(const ctrl::Pose& ref_p, const ctrl::Pose& ref_v,
                 const ctrl::Pose& ref_a) override;
  ctrl::Pose CalcInput(const ctrl::Pose& cur_pos,
                       const ctrl::Pose& cur_v) override;
  void Reset() override;
};
...
class DynamicFeedback : public TrackerBase {
 ...
 public:
  explicit DynamicFeedback(const float omega_n, const float zeta,
                           const float control_period)
      : kp(omega_n * omega_n),
        kd(2.0 * zeta * omega_n),
